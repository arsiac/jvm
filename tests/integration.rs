...
fn create_fake_jdk(base: &std::path::Path, name: &str, version: &str) -> PathBuf {
    ...
    let java_path = if cfg!(windows) { bin_dir.join("java.exe") } else { bin_dir.join("java") };
...
fn test_add_with_custom_alias() {
    ...
    let output = jvm_cmd()
        .arg("add").arg(&jdk)
        .arg("--alias").arg("lts").arg("--alias").arg("latest")
        .env("JVM_DIR", &jvm_dir)
        .output().unwrap();
    assert!(output.status.success());
...
fn test_current_no_active() {
    ...
    jvm_cmd().arg("add").arg(&jdk).env("JVM_DIR", &jvm_dir).output().unwrap();
...
fn test_use_and_current() {
    ...
    let output = jvm_cmd()
        ...
        .env("JVM_DIR", &jvm_dir)
        .output().unwrap();
    assert!(output.status.success(), "use failed: {}", String::from_utf8_lossy(&output.stderr));
...
fn test_remove_active_jdk_rejected() {
    ...
    jvm_cmd().arg("add").arg(&jdk).env("JVM_DIR", &jvm_dir).output().unwrap();
    jvm_cmd().arg("use").arg("17.0.2").env("JVM_DIR", &jvm_dir).output().unwrap();
...
fn test_init_all_shells() {
    ...
    for shell in &["bash", "zsh", "fish", "powershell"] {
        let output = jvm_cmd()
            ...
            .env("JVM_DIR", &jvm_dir)
            .output().unwrap();
        assert!(output.status.success(), "init {} failed: {}", shell, String::from_utf8_lossy(&output.stderr));
        assert!(!String::from_utf8_lossy(&output.stdout).is_empty(), "init {} empty", shell);
    }
}
...
fn test_alias_add_and_remove() {
    ...
    let output = jvm_cmd()
        .arg("alias").arg("add").arg("17.0.2").arg("my-jdk")
        .env("JVM_DIR", &jvm_dir)
        .output().unwrap();
    assert!(output.status.success(), "alias add failed: {}", String::from_utf8_lossy(&output.stderr));
    assert!(String::from_utf8_lossy(&output.stdout).contains("my-jdk"));

    let output = jvm_cmd()
        .arg("alias").arg("remove").arg("17.0.2").arg("my-jdk")
        .env("JVM_DIR", &jvm_dir)
        .output().unwrap();
    assert!(output.status.success());
    assert!(String::from_utf8_lossy(&output.stdout).contains("my-jdk"));
...
fn test_multiple_jdks_and_switch() {
    ...
    for jdk in &[&jdk8, &jdk17, &jdk21] {
        let output = jvm_cmd().arg("add").arg(jdk).env("JVM_DIR", &jvm_dir).output().unwrap();
        assert!(output.status.success(), "add failed: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    for (version, alias) in &[("1.8.0_492", "8"), ("17.0.2", "17"), ("21.0.1", "21")] {
        let output = jvm_cmd()
            ...
            .env("JVM_DIR", &jvm_dir)
            .output().unwrap();
        assert!(output.status.success(), "use {} failed: {}", alias, String::from_utf8_lossy(&output.stderr));
        ...
        assert_eq!(String::from_utf8_lossy(&output.stdout).trim(), *version,
            "current mismatch after switch to {}", alias);
    }
}
...
fn test_add_update_existing() {
    ...
    let output = jvm_cmd()
        ...
        .env("JVM_DIR", &jvm_dir)
        .output().unwrap();
    assert!(output.status.success(), "update add failed: {}", String::from_utf8_lossy(&output.stderr));
}
